...
public static class GetEtfInfoTool
{
    ...
    [Description("""
        Returns an ETF-specific snapshot for a Korean ETF or ETN: price + NAV + tracking-index value + premium/discount (괴리율) + AUM + LP list + 52-week and year ranges + related futures.

        USE WHEN: the user asks about an ETF/ETN (e.g. "KODEX 200 NAV", "TIGER 미국S&P500 괴리율", "ETF 시가총액", "LP 누구야?", "추적기준지수"), or when a `ls_search_stock` result has `etf != "0"`.
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var payload = new
            {
                ...
                etf_kind = b.ReadString("etf_kind"),
                ...
                listing_shares = b.ReadLong("listing"),
