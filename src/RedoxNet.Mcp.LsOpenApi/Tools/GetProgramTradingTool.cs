...
public static class GetProgramTradingTool
{
    ...
    [Description("""
        Returns program-trading (프로그램매매) flow for KOSPI / KOSDAQ via LS TRs. Program trades are basket / arbitrage orders — largely an institutional footprint.
        ...
        Non-arbitrage (비차익) net buying is the institutional-basket signal; arbitrage (차익) is basis-driven and mechanical. In ranking scope, mktcap_ratio (net buying ÷ market cap) is the normalized footprint — a small-cap with a high ratio is a stronger signal than a large-cap with a big absolute number. sort='mktcap_weight' ranks by that footprint directly.
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var payload = new
            {
                ...
                chart_available = includeChart,
                note = "Each row is one day's program net buying (net = arbitrage + non_arbitrage), in 억원. non_arbitrage (비차익) is the institutional-basket signal.",
            };
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var payload = new
            {
                ...
                chart_available = includeChart,
                note = $"Rows are LS-sorted (rank-ascending). net/buy/sell are program-trading values in {valueUnit}; mktcap_ratio is net buying as % of market cap — the normalized institutional footprint.",
            };
