...
public static class GetEtfHoldingsTool
{
    ...
    /// <param name="limit">Cap on the constituents returned, from the top of the LS-sorted list. Defaults to 20; pass -1 for the full list.</param>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (limit == 0 || limit < -1)
            return McpJson.ErrorResult("limit must be a positive integer, or -1 for the full list.");
        ...
        try
        {
            ...
            // limit = -1 is the opt-in "full list" sentinel (consistent with
            // themes_limit on ls_holdings_list); a null cap means no truncation.
            int? cap = limit < 0 ? null : limit;
            ...
            if (holdingsBlock is { ValueKind: JsonValueKind.Array })
            {
                ...
                {
                    if (cap is int capMax && holdings.Count >= capMax)
                        break;
                    holdings.Add(new
                    {
