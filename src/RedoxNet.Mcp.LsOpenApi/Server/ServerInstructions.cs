...
internal static class ServerInstructions
{
    ...
    public const string Text = """
        ...
        1. Local paper portfolio (ls_holdings_list, ls_watchlist, ls_watched_themes, ls_holding, ls_account, ls_portfolio_io) -- the user's manually-tracked book in portfolio.db. Useful for paper portfolios, multi-broker tracking (LS + Kiwoom + Mirae Asset + ...), watchlists, and theme/sector tracking. Does NOT require LS credentials and does NOT auto-sync with the user's real brokerage -- it reflects only what the user entered. A missing or expired LS key only drops live-price enrichment. The `broker` field on these rows is purely a display label -- "한투", "유안타증권", "LS증권 페이퍼" are all valid paper labels. Paper portfolios are LS-mode agnostic: they stay visible regardless of which LS appkey pair (real or virtual) is currently loaded.

        2. LS broker live -- the ls_account_* family: ls_account_holdings (current positions), ls_account_orders (today's fills + pending), ls_account_balance (cash / orderable / total valuation), ls_account_bep (break-even prices), ls_account_order_history (per-day order/fill log), ls_account_transactions (deposits/withdrawals/trades over a date range), ls_account_performance (period P&L), ls_account_daily_pnl (single-day trade log + fees), ls_account_credit_limit (margin loan limits), ls_account_max_order_qty (inquiry-only orderable capacity). Each call is a fresh REST snapshot of the appkey-bound LS subaccount -- no caching, no daemon, no background sync. Each tool takes NO `account` argument: LS account-inquiry TRs do not accept AcntNo in the request, the appkey's authenticated session resolves to one subaccount server-side and only echoes the resolved number in the response. The first ls_account_* call against a fresh registry auto-discovers that subaccount from the response and stores it in a separate `ls_accounts` table -- physically distinct from the paper-portfolio table above, so a paper "LS증권" label never shadows a live row. Friendly label: ls_account action="set_live_nickname".

        The two sources are intentionally allowed to differ -- a paper portfolio in source 1 doesn't change source 2, and vice versa. Route by intent: "내 보유 종목" / "my holdings" without LS context -> ls_holdings_list (paper). "LS 계좌 잔고" / "실계좌" / "오늘 주문 체결" / "예수금" / "수익률" -> the ls_account_* family. LS REST is appkey-routed -- the appkey/appsecretkey pair determines which broker account answers; LS_MARKET is informational (it tags rows so real-mode and virtual-mode (모의투자) registrations stay separate) but does NOT switch the REST endpoint. To target a different account, swap to that account's appkey pair.
