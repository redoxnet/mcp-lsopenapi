...
internal static class ServerInstructions
{
    ...
    public const string Text = """
        ...
        The two sources are intentionally allowed to differ -- a paper portfolio in source 1 doesn't change source 2, and vice versa. Route by intent: "내 보유 종목" / "my holdings" without LS context -> ls_holdings_list (local). "LS 계좌 잔고" / "실계좌" / "오늘 주문 체결" / "예수금" / "수익률" -> the ls_account_* family. LS REST is appkey-routed -- the appkey/appsecretkey pair determines which broker account answers; LS_MARKET is informational (it tags portfolio.db rows so real-mode and virtual-mode (모의투자) registrations stay separate) but does NOT switch the REST endpoint. To target a different account, swap to that account's appkey pair. The first ls_account_* call against an empty registry auto-discovers the broker account from the response's AcntNo and records it -- no manual upsert needed.
