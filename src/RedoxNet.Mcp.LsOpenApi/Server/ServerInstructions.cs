...
internal static class ServerInstructions
{
    ...
    public const string Text = """
        ...
        1. Local paper portfolio (ls_holdings_list, ls_watchlist, ls_watched_themes, ls_holding, ls_account, ls_portfolio_io) -- the user's manually-tracked book in portfolio.db. Useful for paper portfolios, multi-broker tracking (LS + Kiwoom + Mirae Asset + ...), watchlists, and theme/sector tracking. Does NOT require LS credentials and does NOT auto-sync with the user's real brokerage -- it reflects only what the user entered. A missing or expired LS key only drops live-price enrichment. The `broker` field on these rows is purely a display label -- "한투", "유안타증권", "LS증권 페이퍼" are all valid paper labels.
