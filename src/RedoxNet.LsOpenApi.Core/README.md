...
| **Catalog** | `TrCatalog.Default` — 64-TR catalog as an embedded resource (시세 / 차트 / 지수 / 업종 / 테마 / ETF / 스크리너 / 종목조회 / 해외주식시세 / 해외주식차트 / 조건검색 / 기타). `Search` ranks by exact-code, name, category, description, and field-level matches. |
| **Trading calendar** | `ITradingCalendar` + in-box `WeekendOnlyCalendar` + `DateEnvelope.Resolve(...)` — normalize a caller `yyyyMMdd` (or `null` = "latest") into `(resolvedDate, resolution)` where `resolution` is one of `used` / `weekend` / `holiday` / `future_date` / `pre_market`. Registered via DI so a holiday-aware calendar can drop in without touching call sites. |
