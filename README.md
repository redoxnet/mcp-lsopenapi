...
API 사용 시 [LS증권 OpenAPI 이용 안내](https://openapi.ls-sec.co.kr/howto-use)를 참조하고, 사이트에 표시되는 정식 이용약관을 확인 후 준수하시기 바랍니다. 현재 범위는 **국내주식 중심의 read-only 시장 데이터 + 해외주식 시세·차트 조회**(해외 지수·환율·선물 스냅샷 포함) **+ 로컬 포트폴리오 노트**입니다. **v1.6에서 LS 실계좌 조회**, **v1.7에서 주문 발주** 가 추가 예정입니다. 실시간 시세(WebSocket) 는 현재 고려 대상이 아닙니다.
