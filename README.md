...
이 서버를 사용하려면 LS증권 OpenAPI 의 **`AppKey`** 와 **`AppSecretKey`** 가 필요합니다. 절차:

1. **LS증권 계좌 개설.** 실투자용은 비대면(앱) 또는 영업점 개설, 모의투자용은 영업점 또는 모의투자 별도 신청 채널.
2. **OpenAPI 사이트 가입.** [openapi.ls-sec.co.kr](https://openapi.ls-sec.co.kr) 회원가입 후 LS증권 계좌와 연결.
3. **앱 등록.** 마이페이지 → "앱 관리" / "OpenAPI 신청" → 앱 이름·콜백 URL·이용 약관 동의 → `AppKey` / `AppSecretKey` 발급.
4. **모의투자 ↔ 실투자 키는 별개.** 모의투자 키로 실서버를 호출할 수 없습니다 (반대도 동일). `LS_MARKET` 값과 키의 발급 환경이 일치해야 합니다.

> 분실 시 마이페이지에서 재확인 / 재발급 가능. **노출 가능성이 있다고 판단되면 즉시 폐기·재발급**하세요. 발급된 키는 일정 기간 미사용 시 LS측에서 비활성화될 수 있으니 주기적으로 정상 동작 여부 확인 권장.
