...
- [x] **v0.10.0** — 도구 표면 압축 (SPEC v0.10) — 마지막 0.x 마이너. 포트폴리오 도구 20종을 action 라우팅 dispatcher 5종(`ls_account`/`ls_watchlist`/`ls_watched_themes`/`ls_portfolio_io`/`ls_holding`)으로 통합, `LS_TOOL_PROFILE`로 카탈로그 3종을 `standard` 프로파일에서 숨김. 48 → 32(`standard`)/35(`all`) 도구. 추가: `ls_get_stock_info` 외국인 보유 섹션(t1716), 리스트 도구 `limit`/`total_available` 통일, `ls_get_index_history` export 모드.
