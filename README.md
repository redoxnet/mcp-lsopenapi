...
Claude·ChatGPT·Copilot 같은 AI 비서에 **LS증권 OpenAPI**를 붙입니다. 시세·차트·기업정보·ETF 구성·시장 스크리너를 평소 쓰던 대화창에서 자연어로 묻고 받습니다. **v0.5부터는 내 보유 종목·관심종목·관심섹터까지 대화로 기록·추적**합니다.

> *"한투에 LG전자 64주 평단 11.5만, 카카오페이엔 10주 9.8만에 샀어"*
> *"민테크 10:1 분할됐대. 내 보유 반영해줘"*
> *"내가 들고 있는 종목 평가손익 어때?"*
...
> *"한투에 LG전자 64주 평단 11.5만에 샀어"* → 자동 등록
> *"5주 더 8만에 추가 매수"* → 가중평균 평단 자동 계산
> *"민테크 10:1 분할됐대"* → 모든 계좌에 일괄 반영
> *"카카오페이 보유분만 보여줘"* → 계좌 필터링
> *"유안타 LG전자 24주 익절"* → 부분 매도, 평단 유지
...
그룹별 책갈피(`반도체-AI` / `2차전지수혜주` 같은 사용자 분류), 테마/섹터 코드(t1531 tmcode)로 평균등락률 추적. 시세 enrich는 LS 자격증명 있을 때 자동.
...
v0.x.x는 **국내주식 read-only 시세 데이터 + 로컬 포트폴리오 노트** 범위입니다. v0.5의 포트폴리오 도구는 수동 입력 기반의 로컬 저장만 지원하며 브로커 계좌 동기화·실주문은 하지 않습니다. 실시간 시세(WebSocket), 실 계좌 조회/주문은 후속 릴리스 예정입니다.
