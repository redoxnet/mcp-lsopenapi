...
### 두 개의 저장 신호를 합쳐 — HTS 한 화면으로는 못 쓰는 조건 (v1.4)
> *"거래대금 급증과 외국인 매수 streak, 둘 다 잡힌 종목만"*

LS의 Q클릭 신호 카탈로그에서 두 개를 골라 한 번에 부르면, 교집합(AND) 또는 합집합(OR)으로 한 번에 처리됩니다. 각 종목 옆에 어느 신호에 함께 잡혔는지(`signals_matched`)가 따라 오기 때문에, *"이 중 외국인 streak이 더 오래된 것부터 보자"* 같은 후속 대화로 그대로 이어집니다 — 별도 조건식을 짤 필요가 없습니다.

### 분할 매수 평단 머지 + 액면분할 일괄 반영
